...
println("DELTA = ", delta);
k = Float64(1.0);                                                   # zmienna, która będzie miała wartości 1,2,...
...
while (Float64(x) <= Float64(p2))                                   # dopuki x jest w przedziale [0.5,1]
    x = Float64(x + k * delta);
    if x - prevfloat(x) != delta
        println("x = ", x, " => DELTA = ", x - prevfloat(x));
    end
...
while (Float64(x) <= Float64(p2))                                   # dopuki x jest w przedziale [2,4]
    x = Float64(x + k * delta);
    if x - prevfloat(x) != delta
        println("x = ", x, " => DELTA = ", x - prevfloat(x));
    end
